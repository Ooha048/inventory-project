...
<div >
	<%
		if (role.equals("EMPLOYEE")) {
			out.print("<a class='createorder' href=" + request.getContextPath() + "/shift-create.jsp>Request</a>");
		}
	%>
	<table class="table table-bordered">
			...
			<tbody>
				...
				<c:forEach var="shift" items="<%=request.getAttribute(\"shifts\")%>">
					<tr>
						<td><c:out value="${shift.id}" /></td>
						<td><c:out value="${shift.newShift}" /></td>
						<td><c:out value="${shift.status}" /></td>
						<td>
						<c:if test="${role=='MANAGER'}">
						<a href="<%=request.getContextPath()%>/shift/edit?id=<c:out value='${shift.id}' />&status=<c:out value='APPROVED' />">
							Approve
						</a>
						&nbsp;&nbsp;
						<a href="<%=request.getContextPath()%>/shift/edit?id=<c:out value='${shift.id}' />&status=<c:out value='REJECTED' />">
							Reject
						</a>
						</c:if>
						<c:if test="${role=='EMPLOYEE'}">
						<a href="<%=request.getContextPath()%>/shift-edit.jsp?id=<c:out value='${shift.id}' />&shiftreq=<c:out value='${shift.newShift}' />">
							Edit
						</a>
						</c:if>
						&nbsp;&nbsp;
						<a href="<%=request.getContextPath()%>/shift/delete?id=<c:out value='${shift.id}' />">
							Delete
						</a>
						</td>
					</tr>
