...
<div >
	<%
		if (role.equals("MANAGER")) {
			out.print("<a class='createorder' href=" + request.getContextPath() + "/task/create>Create Task</a>");
		}
	%>
	<table class="table table-bordered">
			...
			<tbody>
				<c:forEach var="task" items="<%=request.getAttribute(\"tasks\")%>">
					<tr>
						...
						<a href="<%=request.getContextPath()%>/task/edit-status?id=<c:out value='${task.id}' />&status=<c:out value='DONE' />">
							Mark it as Done
						</a>
						&nbsp;&nbsp;
						</c:if>
						<c:if test="${role=='MANAGER'}">
						<a href="<%=request.getContextPath()%>/task/edit-task?id=<c:out value='${task.id}' />&task=<c:out value='${task.task}' />">
							Edit
						</a>
						</c:if>
						&nbsp;&nbsp;
						<c:if test="${role=='MANAGER'}">
							<a href="<%=request.getContextPath()%>/task/delete?id=<c:out value='${task.id}' />">
								Delete
							</a>
						</c:if>
						</td>
